...
public class FirUpdaterUtils {
    ...
    public static Intent getInstallApkIntent(Context context, String apkPath) {
        try {
            ...
            String comm = "chmod 777 " + apkFile.getAbsolutePath();
            Runtime.getRuntime().exec(comm);
            Uri apkUri;
            Intent intent;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                apkUri = FileProvider.getUriForFile(context, getPackageName(context) + ".update.fileProvider", apkFile);
    ...
    public static void putCurrLengthValue(Context context, String key, int value) {
        getSharedPreferences(context)
                .edit()
                .putInt("curr_" + key, value)
                .apply();
    }

    public static int getCurrLengthValue(Context context, String key) {
        return getSharedPreferences(context).getInt("curr_" + key, 0);
    }
    public static void putFileLengthValue(Context context, String key, int value) {
        getSharedPreferences(context)
                .edit()
                .putInt("file_" + key, value)
                .apply();
    }

    public static int getFileLengthValue(Context context, String key) {
        return getSharedPreferences(context).getInt("file_" + key, 0);
    }
